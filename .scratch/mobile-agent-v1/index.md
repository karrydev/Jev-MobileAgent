...
2026-09-22：用户已确认本规格及 28 项拆分。唯一执行队列为本目录；旧 mobile-agent-development 的 21 项保留为来源，不再领取、不改写为完成。
...
| 21 | [提供 Jev 实测访问条件](issues/21-human-jev-access.md) | 无 | 用户访问条件 |
| 22 | [真实 Jev 在 VLM 任务旁提供可评估建议](issues/22-jev-shadow.md) | 11, 19, 21 | 真实模型、物理手机影子运行 |
...
| 26 | [提供最终服务器与网络访问条件](issues/26-human-server-access.md) | 无 | 用户访问条件 |
| 27 | [拔掉 USB 后从手机完成服务器任务](issues/27-server-no-adb.md) | 20, 26 | 服务器与物理手机 |
| 28 | [最终集成版本通过完整对照与交付复核](issues/28-final-integrated-acceptance.md) | 25, 27 | 最终服务器与物理手机 |

## 开始与用户参与

初始 Agent 队列为 01 模拟任务闭环、02 独立判分，两个任务可并行。01 合入后按依赖推进控制、设备、模型回放；01/02 合入后可推进离线策略。共享协议与根配置由协调者指定唯一负责人。

先尽量完成 01–12 的离线和模拟器工作。13 VLM 与 14 手机在探针/安装包就绪、确需真实基线时尽量合并请求；21 Jev 在 11/19 完成后请求；26 服务器在 20 真机恢复及本地部署准备通过后请求。这四项无技术前置，但属于 ready-for-human，不加入自动领取队列，也不要求用户现在准备。

真实验证的硬依赖保留：15 → 16 → 17 → 18 → 19；19 后恢复与 Jev 可以并行；最终在 28 汇合并重新部署最终版本验收。模拟器实际不可用时记录阻塞，继续其他离线任务，不将 Mock 当作设备证据。
