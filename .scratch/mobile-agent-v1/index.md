...
2026-09-22：用户已确认本规格及 28 项拆分。2026-09-25：按独立 App 要求修订，编号仍为 01–28；01–20 保留原环境验收，21–28 共 8 项待完成。唯一执行队列为本目录；旧 mobile-agent-development 的 21 项保留为来源，不再领取、不改写为完成。
...
当前可领取 **21（Jev 访问探针）和 26（手机内 VLM 闭环）**。用户已提供统一测试凭据 `jev-test.env`，不再请求服务器资源。26 不依赖 Jev，可先完成 App 内仅 VLM；26 合入后，22 影子策略与 27 恢复验证可按代码所有权协调推进，之后 22 → 23 → 24 → 25，最终 25/27 → 28。
...
图仅展示待办依赖；已完成的 11/12/19/20 前置保留在票头。21 与 26 可并行，涉及同一 Android 运行入口/持久化的 22 与 27 修改必须指定唯一所有者或串行；同一真机测试串行进行。编号是稳定身份，不等于执行顺序，不为调整架构扩张任务数量。
