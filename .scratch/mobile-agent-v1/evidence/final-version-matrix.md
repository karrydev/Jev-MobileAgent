...
| 按需规划 | 2915ed9；最终版本待定 | 四项同APK复测1项自动成功，未达到质量放行；全部尝试保留 | on-demand-planning-review.md、on-demand-planning-plan.md |
| 独立恢复 | 多个精确版本见各JSON，最终修复待集成 | 三种进程窗口、断网、锁屏、撤权和真实重启已留证；锁屏停止及显式恢复已留证，原始触摸覆盖有限；最新现场误暂停仍待修复 | standalone-recovery-validation.md、recovery27-*.json |
