...
后续现场门禁复验（真实调用前冻结）：新APK在Manager请求在途期间通过Android真实切换到系统设置，不点击配置项；应以现场变化暂停，不发设备动作，迟到回复仅结算usage。切回原目标仍不得自动恢复，fresh核对、确认前重新采集与明确用户决定后才继续/结束。manual01的已聚焦空框触摸不作为此项的旧失败证明。光标重采样使用仍保留焦点的中文框，不以强制去焦点替代新门禁；语义/目标变化应拒绝，有限采样必须留完整观察/截图关联。对运行自身造成的跨应用A→B另核对可信目标推进，不能用人工跳页冒充Agent正常导航。
