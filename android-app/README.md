...
开启“Jev 受控候选选择”后，新任务会固定该设置。原 VLM Executor 仍按既有频率提出动作；只有该动作能唯一匹配当前观察中的完整候选时，才请求 Jev 并执行合法选择。候选覆盖不足、建议无效或置信度不足时沿用原 VLM 动作，预算门控拒绝时暂停。实际执行动作进入原有设备门禁、核验和历史；同时开启影子开关也不会多发一次请求。

本阶段只改变选择策略，不改变规划和视觉核验频率，也不承诺减少调用。当前受控验收状态见[任务23](../.scratch/mobile-agent-v1/issues/23-jev-controlled.md)，配对规则见[冻结计划](../.scratch/mobile-agent-v1/evidence/jev-controlled-plan.md)。
