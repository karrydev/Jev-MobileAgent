...
核查日期：2026-09-22。本文为准备清单；未创建远程仓库、购买额度、安装依赖或调用付费模型。当前选型为 v3.5、公开 Fork 后裁剪、按需整版升级或择取补丁，完整决定见[项目方向](../project-direction.md)。
...
当前本地 `Jev-MobileAgent` 目录包含已有研究文档，尚未初始化为 Git 仓库。开始时保留这些文档，将其合入新的工作副本，不在已有非空目录直接覆盖 clone。已审阅上游基线为 `11cea575561fb7800b5fb6b6cafa56f7a91de11f`；正式开始时记录实际选用提交。
