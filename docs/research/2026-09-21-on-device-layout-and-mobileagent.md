...
资料核查日期：2026-09-21；方案更新：2026-09-22。手机 App 自行采集其它 App 的界面并执行操作，连接服务端编排和模型，使用时无需电脑。项目先自用、从开始公开、不上应用商店；基于 v3.5 公开 Fork 后裁剪，详见[当前决策](../project-direction.md)。尚未安装 App、启用权限、调用 Jev 或运行真机闭环。

## 判断

**手机端直接实现 AccessibilityService 最贴近目标；Shizuku + UiAutomation 是有条件的补充。MobileAgent 可以作为编排底座，复用规划、执行反馈和记忆；仍需新增手机设备适配层和 Jev 决策层。**

“不依赖电脑”与“全部控制逻辑运行在 APK 内”是两个要求。App 通过互联网连接服务端 MobileAgent，也可脱离电脑运行，且更容易复用 Python 代码。如果控制循环也必须在手机内，需移植或封装 Python 运行时，并重新解决 Android 生命周期和依赖。
...
保留原模型的生成能力，才能复用规划、摘要和视觉反思；仅替换一个模型配置不会自动获得上述新增功能。Python 编排先在开发机运行，后迁至普通服务器，使用时 App 无需连接电脑。
