...
采用 MobileAgent v3.5：`mobile_use` 是单模型真机参考，所需四角色逻辑从 `android_world_v3.5` 提取并解除环境耦合。设备接口需新增，不能假设 v3.5 已有可直接替换的 App Controller。手机 App 通过无障碍提供观察和执行，按 ADR-0003 将控制循环迁入 App，Python 仅保留为开发参考。Jev 承担候选选择及闭集核验，原生成式模型承担规划、内容生成和视觉兜底。Jev 不接收截图，[输入边界见官方文档](https://docs.typesafe.ai/models)。
...
| 5：完整闭环与真机复核 | 组合各模块，检查断线恢复、进程重启、用户手动操作与停止/恢复；在真实中文 App 运行 | 配对比较原版、仅设备桥、加 JevSelector、加 Verifier、加按需规划。模拟器用独立判分；真机按任务 rubrics 留存证据。拔掉 USB 并停止主机 ADB 后验证独立 APK；同时关闭无线调试、停止所有项目编排服务 | AndroidWorld/MobileWorld；Mobile-Eval-E 任务；自建中文真机集 |
