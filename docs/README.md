...
更新日期：2026-09-22。公开 Fork、工程技能配置与 CodeGraph 已初始化，正式规格与本地任务已建立，模拟任务闭环、独立离线判分和离线选择/核验已通过验收；Android 观察已在模拟器与物理手机受控页面验证；确定性节点动作与任务控制已在真机验证；截图与手势也已在受控真机验证。真实模型基础访问已通过，原版手机协议与最小坐标适配后的四角色协议验证已通过；原版双入口基线已采集（真机成功、AndroidWorld 5 步任务未完成）；App+VLM 闭环及代码裁剪尚未验收。
